.PHONY: all build check clean dist clean-man deps deps-develop deps-test install install-man integration lint man package prettier prettier-fix spec-coverage test tidy tidy-fix uninstall upgrade vm-provision vm-up web web-clean
...
# META.json, MANIFEST, lib/ and the test tiers that run from a dist
# tree. CI attaches it to each release, and PAUSE indexes it.
dist:
