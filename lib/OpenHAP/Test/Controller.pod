...
L<Protocol::HAP::HTTP> builds the requests and parses the responses. The
controller thus reads the same wire format that the server writes, and
no second HTTP parser can drift from the first.
...
Protocol::HAP::Server engine, without sockets. The request bytes and the
...
L<Protocol::HAP::HTTP>, L<OpenHAP::Test::Controller::SRP>,
L<OpenHAP::Test::Integration>, F<spec/HAP-Pairing.md>,
F<spec/HAP-Encryption.md>
