...
The method constructs an L<Protocol::HAP::Controller> for the
...
L<Protocol::HAP::HTTP> builds the request and frames the response. The
method reads until the response is whole, and it refuses a response
over 1 MB.
...
The method gets a configuration value. L<FuguLib::Config> reads the
installed F</etc/openhapd.conf>, thus the tests see the file as the
daemon sees it.
...
L<FuguLib::Config>, L<Protocol::HAP::HTTP>, L<Protocol::HAP::Controller>
