...
L<FuguLib::Pidfile>. Those two modules document the behaviour. New code
...
The library directories come from C<FuguLib::Sandbox-E<gt>perl_lib_dirs>,
which reads C<%Config>. They never come from the live C<@INC>. The
shared read-only entries - the resolver files, the service tables, the
time zone and the random device - come from
C<FuguLib::Sandbox-E<gt>system_paths>. The method includes
C<script_lib> read-only, and only when that directory holds an
F<OpenHAP/> directory. Thus an installed layout never unveils
F</usr/local/lib>.
...
L<FuguLib::Daemon>, L<FuguLib::Pidfile>, L<FuguLib::Sandbox>, L<rc.d(8)>
