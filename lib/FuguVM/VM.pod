...
FuguVM::VM - the lifecycle of one OpenBSD virtual machine
...
Everything that is not QEMU comes from FuguLib. Processes and liveness
come from L<FuguLib::Process>, which reaps: a QEMU that became a
zombie reads as stopped, and a caller that is about to start a second
one needs that answer. Bounded waits come from L<FuguLib::Util>. The
random root password comes from L<FuguLib::Crypto>. The guest
connection comes from L<FuguLib::SSH>.

=head1 METHODS

=head2 new

    FuguVM::VM->new(config => \%vm, state => $state, log => $log, ...)
...
C<down> stops the VM gracefully and records a clean shutdown.
C<destroy> stops it and deletes the disk. C<start> and C<stop> are the
narrow forms that do not touch the proxy. C<status> reports the state.

=head2 wait_ssh, console_port, is_running

C<wait_ssh> polls until the guest takes an authenticated connection.
C<console_port> is the port that C<telnet> reaches the serial console
on.

=head1 SHUTDOWN

A shutdown syncs the guest filesystems first, then asks the guest to
power off through ACPI, then through the guest agent, and only then
forces the process to stop. Every step is bounded: a guest that stops
answering must not hold the caller.
...
L<FuguLib::Process>, L<FuguLib::SSH>, L<FuguLib::Util>,
L<FuguVM::Disk>, L<FuguVM::ImageCache>, L<FuguVM::QGA>,
L<FuguVM::QMP>, L<FuguVM::State>, L<fuguvm(1)>
