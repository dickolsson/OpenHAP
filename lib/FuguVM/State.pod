...
The JSON blob rides on L<FuguLib::Store>. The two process IDs ride on
L<FuguLib::Pidfile>, which takes its lock before it truncates and
...
the proxy lifecycle belongs to L<FuguVM::VM>. That split is what
...
=head2 new
    ...
    FuguVM::State->new($state_dir, $vm_name)
...
C<store> returns the L<FuguLib::Store>. L<FuguVM::VM> gives it to the
...
Return the L<FuguLib::Pidfile> objects. QEMU writes the first itself,
through its C<-pidfile> option; the proxy supervisor owns the second.

=head2 set_vm_pid, get_vm_pid, clear_vm_pid, is_vm_running

The QEMU process.

=head2 set_proxy_pid, get_proxy_pid, clear_proxy_pid, is_proxy_running

The proxy child.

=head2 set_proxy_port, get_proxy_port, clear_proxy_port
...
=head2 is_ssh_key_installed, mark_ssh_key_installed, get_installed_ssh_pubkey, ssh_key_matches
...
L<FuguLib::Pidfile>, L<FuguLib::Store>, L<FuguVM::VM>
