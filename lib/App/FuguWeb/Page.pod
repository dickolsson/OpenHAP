...
=head2 document
    ...
    my $bytes = $page->document($title, $fragment);

Return the whole page as bytes. The fragment goes in unchanged: it is
already HTML, from a renderer or from the project's own source
directory.
...
Write the page with L<Fugu::File>. The method returns true on success,
and C<undef> with a message in the log otherwise.

=head1 OPTIONAL FILES

Both live in the configured source directory.

=over 4

=item F<footer.body.html>

The footer prose. It is project text, not tool text, so it is content
and not a setting. A project without it gets no C<< <footer> >> element
and no rule before one: an empty footer is a gap, not chrome.

=item F<extra.css>

A project stylesheet. The chrome links it after the base sheet, so a
project rule wins over the rule it overrides. The asset rule of
L<App::FuguWeb::Site> copies it.
