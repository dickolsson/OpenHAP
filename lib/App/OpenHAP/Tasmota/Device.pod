...
The Tasmota C<FullTopic> pattern. The default is C<%prefix%/%topic%/>.
The class substitutes C<%prefix%> and C<%topic%> for every topic it
builds.

=item C<setoption26>

True when the device runs with C<SetOption26>. Tasmota then uses the
indexed C<POWER1> name even for a device with one relay.
...
The class publishes to C<cmnd/Power>, C<cmnd/Status>, and
C<cmnd/TelePeriod>. The power topic carries the relay index, or the
C<SetOption26> name, when either applies.
...
=head2 get_availability()

Return the availability as C<AVAILABILITY_UNKNOWN>,
C<AVAILABILITY_ONLINE>, or C<AVAILABILITY_OFFLINE>.

=head2 set_power($state), toggle_power(), blink($on)

Publish a power command. C<$state> is 1 for on and 0 for off.

=head2 query_status($type), force_telemetry()

Ask the device for a status report, or for an immediate telemetry
message.
...
Each one does nothing by default, so a subclass takes only what it
needs.

=over 4

=item C<_process_state_data($data)>

The decoded C<STATE> or C<STATUS11> payload. The default caches it and
extracts the power state, so an override calls C<SUPER> first.

=item C<_process_result_data($data)>

The decoded C<RESULT> payload. The default extracts the power state.

=item C<_process_sensor_data($data)>

The decoded C<SENSOR> payload.

=item C<_on_power_update($state)>

The power state changed. C<$state> is 1 for on and 0 for off.

=item C<_on_availability_changed($old, $new)>

The availability changed. Both arguments are C<AVAILABILITY_*> values.

=back

=head1 SEE ALSO

L<Protocol::HAP::Accessory>, L<App::OpenHAP::Devices>,
F<spec/MQTT-Transport.md>, F<spec/MQTT-Control.md>,
F<spec/MQTT-State.md>
