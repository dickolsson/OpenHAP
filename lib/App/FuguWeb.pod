...
This file holds what every module in the namespace needs. The rest of
the work is split by concern:
...
Three rules keep a list out of the file. A C<manuals> block globs its
directory. A C<modules> block finds every F<.pod> file below its
directory. Every file in the source directory that does not end in
F<.body.html> is an asset, and the build copies it.
