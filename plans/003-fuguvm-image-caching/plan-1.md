...
Introduce the native installed-image cache: a new `FuguVM::ImageCache` module,
backing-file support actually used by `FuguVM::Disk`, and save/restore wiring in
`FuguVM::VM::up`. After this phase, any machine with a warm `cache_dir` boots an
installed VM without downloading the miniroot or running the installer.

## Tasks

### 1.1 `Disk::create` backing format

- `FuguVM::Disk::create` already accepts `$backing_image` but hardwires `-F raw`
  (`lib/FuguVM/Disk.pm:44`). Add a backing-format parameter (the cache passes
  `qcow2`) and keep the current default for compatibility.
...
### 1.2 `FuguVM::ImageCache`

New module (plus `.pod` sidecar and `t/fuguvm/imagecache.t`):

- `new($cache_dir)` — expands `~` like `FuguVM::Image` does.
- `key($vm_config)` — returns `<version>-<arch>-<hash8>`; `hash8` is a truncated
  `Digest::SHA` SHA-256 over version, `FuguVM::Image::ARCH`, `disk_size`, the
  bytes of the `install.exp` that `FuguVM::Expect` resolves, and the contents of
  `share/fuguvm/cache-generation` (bump that file when install-driver behavior
  changes outside `install.exp`). Serialize the inputs with an unambiguous
  delimiter. The generation counter is a data file rather than a constant in
  `ImageCache.pm` so a `hashFiles()` CI key can observe it: `actions/cache` runs
  no save step on a primary-key hit, so a generation bump the workflow key
  cannot see would rotate fuguvm's key, miss, reinstall, and then discard the
  result on every run until some unrelated hashed file changed. Expose the
  script path from `FuguVM::Expect` via a small public method instead of
  duplicating its search logic.
...
- **Restore:** in the no-disk path, consult `lookup` before creating a blank
  ...
  `cached_from` key for diagnostics), and skip `FuguVM::Image->ensure` entirely
...
- **Chain check:** when a disk already exists, verify with `qemu-img info` that
  ...
  image missing from cache; run `fuguvm destroy` and `fuguvm up`" instead of
  ...
  and prints "run 'fuguvm disk repair'" — a dead end, since
...
`VM::_cache_dir` (`VM.pm:967-971`) hardcodes `$ENV{HOME}/.cache/fuguvm`. The
...
hardcoded `path: ~/.cache/fuguvm` documents the default only.

### 1.5 One CI cache entry for the disk and its base

Phase 4 retires `.fuguvm` caching entirely; until then a cached `.fuguvm`
contains an overlay whose absolute backing reference points into
`~/.cache/fuguvm/installed/<key>/`. Coupling the keys of the two existing
...
LRU-evicted independently. Restore `.fuguvm` without `~/.cache/fuguvm` and the
new chain check fails the job, `scripts/vm-up` runs under `set -e`, no step runs
the `fuguvm destroy` the message asks for, and the cache post-step does not save
on a failed job — so every rerun fails identically until someone deletes the
entry out of band.

In `.github/workflows/integration.yml`:

- Replace the two cache steps with ONE `actions/cache` step listing both paths
  (`~/.cache/fuguvm` and `.fuguvm`) under one key, so presence is atomic and an
  overlay is never restored without its base.
- Key it on `runner.arch` plus
  `hashFiles('.fuguvmrc', 'share/fuguvm/expect/install.exp', 'share/fuguvm/cache-generation', 'deps/OpenBSD.txt', 'scripts/vm-provision')`,
  ...
  restored `.fuguvm` means `ImageCache::key` is never called at all.
...
- `lib/FuguVM/ImageCache.pm` + `lib/FuguVM/ImageCache.pod`
- `share/fuguvm/cache-generation`
- Changes to `lib/FuguVM/Disk.pm`, `lib/FuguVM/VM.pm`, `lib/FuguVM/Expect.pm`
  (script-path accessor), `lib/FuguVM/Config.pm` (`cache_dir` injection)
- `t/fuguvm/imagecache.t`; extended `t/fuguvm/disk.t`, `t/fuguvm/vm.t` and
  `t/fuguvm/config.t`
...
- On a host with QEMU: `fuguvm up` (cold) installs and populates
  `installed/<key>/`; `fuguvm destroy && fuguvm up` (warm) reaches "VM ready"
  with no installer run and no miniroot download, and `fuguvm ssh` works —
  key-installed via the seeded root password.
- Deleting the cached base under a stopped VM's existing overlay produces the
  chain-check error and its `fuguvm destroy` remedy — not "Disk corruption
  detected", and not an opaque QEMU failure.
- With `cache_dir` set to a non-default path, `up` populates that path and
  nothing under `~/.cache/fuguvm`.
