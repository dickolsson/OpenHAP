...
workflow-level disk archiving. `.fuguvm` becomes fully ephemeral in CI; the only
cached artifact is `~/.cache/fuguvm`.
...
- `scripts/vm-provision` computes a provisioning key — a short SHA-256 over
  ...
  1. **Deps layer (cacheable):** `pkg_add -u`, cpanm bootstrap, `make deps`.
     Attempted first via `fuguvm snapshot restore deps-<hash>`; on a miss, run
     the deps layer, then `fuguvm down`, `fuguvm snapshot save deps-<hash>`,
     `fuguvm up`, `fuguvm wait`. The stop/start cost is paid only when
...
- **No handoff variable.** `vm-up` and `vm-provision` are sibling make recipes
  ...
  last step of the deps layer, before `fuguvm down`, e.g.
  `vm_run 'test -f /var/db/fuguvm-deps-<hash>'`. Guest-side rather than
...
- The restore attempt still belongs in `scripts/vm-up` before `fuguvm up`,
  guarded to run only when no working disk exists yet. Keep the split simple and
  legible — the scripts own this policy, fuguvm only provides the verbs.
...
- Drop `.fuguvm` from the merged cache step phase 1 created, leaving a single
  step caching `~/.cache/fuguvm` alone:
  - `key`: runner arch + the manual `v<N>` lever +
    `hashFiles('.fuguvmrc', 'share/fuguvm/expect/install.exp', 'share/fuguvm/cache-generation', 'deps/OpenBSD.txt', 'scripts/deps', 'scripts/vm-provision')`
  - `restore-keys`: the arch-scoped prefix, so a key rotation still restores the
    miniroot, still-valid bases, and lets fuguvm's own keying decide what is
    reusable.
  - State the save rule precisely, because it constrains the key: the post-step
    ...
    not a constant.
- Run `./bin/fuguvm cache clear --stale || true` in the post-test cleanup step
...
- Drop the `.fuguvm`-specific choreography that existed only for archiving: the
  socket `find -delete` and the cache-consistency comments. Keep the
  `fuguvm down` step itself — a clean shutdown is still correct hygiene and
...
- Keep the ephemeral-SSH-key step, and correct its comment rather than extending
  it: the key is still written to the global `~/.fuguvmrc` to keep the project
  `.fuguvmrc` hash stable, since that file is still hashed into the surviving
  cache key. What changes is only that no `.fuguvm` state directory is archived.
  The root password is not thereby removed from the cache — it moves into
  `meta.json` under `~/.cache/fuguvm`, which is still saved.
...
- Update the `integration-tests` skill's cold-cache procedure again: phase 1
  ...
  reference stays in `fuguvm.1` and the workflow file.
...
- A run after touching `deps/OpenBSD.txt` or `scripts/deps` re-runs only the
  ...
  `install.exp` or `share/fuguvm/cache-generation` re-runs the OS install (new
  base key).
- `.fuguvm` no longer appears in any `actions/cache` path; no run depends on a
  previous run's state directory.
- `make integration` works on a developer machine with a persistent `.fuguvm`,
