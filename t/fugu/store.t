...
subtest 'set, get and persist' => sub {
	my $path  = next_path();
	my $store = Fugu::Store->new( path => $path )->load;
...
subtest 'increment' => sub {
	my $store = Fugu::Store->new( path => next_path() )->load;
	...
	is( Fugu::Store->new( path => $store->path )->load->get('c'),
		7, 'and on disk' );
};

subtest 'load tolerates a missing and a corrupt file' => sub {
	my $missing = Fugu::Store->new( path => "$dir/never-written.json" );
	...
	my $corrupt = Fugu::Store->new( path => $path );
...
subtest 'save is atomic' => sub {
	...
	my $unencodable = Fugu::Store->new( path => $path )->load;
	...
	is( Fugu::Store->new( path => $path )->load->get('good'),
...
subtest 'the mode keeps the state private' => sub {
	my $path = next_path();
	Fugu::Store->new( path => $path )->load->set( 'secret', 'value' );
	is( ( stat $path )[2] & 07777, 0600, 'the default mode is 0600' );

	my $open = next_path();
	Fugu::Store->new( path => $open, mode => 0644 )->load
	    ->set( 'public', 'value' );
...
subtest 'a missing path is a programming error' => sub {
	ok( !eval { Fugu::Store->new; 1 },              'new needs a path' );
	ok( !eval { Fugu::Store->new( path => '' ); 1 }, 'and a real one' );
};
