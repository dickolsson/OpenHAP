...
subtest 'bounded returns what the code returns' => sub {
	is( FuguLib::Util::bounded( 5, sub { 'value' } ),
		'value', 'the return value passes through' );
	is_deeply( FuguLib::Util::bounded( 5, sub { [ 1, 2 ] } ),
		[ 1, 2 ], 'a reference passes through' );
	is( FuguLib::Util::bounded( 5, sub { undef } ),
		undef, 'undef passes through' );
};

subtest 'bounded stops a call that runs too long' => sub {
	my $start = time;
	my $result = FuguLib::Util::bounded( 1, sub { sleep 30; 'never' } );
...
subtest 'bounded lets a real error through' => sub {
	ok( !eval { FuguLib::Util::bounded( 5, sub { die "real failure\n" } ); 1 },
...
subtest 'wait_until polls until the condition holds' => sub {
	my $calls = 0;
	my $result = FuguLib::Util::wait_until( 5, 0.05, sub { ++$calls >= 3 } );
...
subtest 'wait_until runs the code at least once' => sub {
	my $calls = 0;
	my $result = FuguLib::Util::wait_until( 0, 0.05, sub { $calls++; 'now' } );
...
subtest 'wait_until gives up at the deadline' => sub {
	my $start = time;
	my $result = FuguLib::Util::wait_until( 0.5, 0.05, sub { 0 } );
	my $elapsed = time - $start;
...
subtest 'wait_until stops on an interrupt' => sub {
	...
	my $result = FuguLib::Util::wait_until( 30, 0.05,
		sub { $calls++; 0 } );
	my $elapsed = time - $start;
...
subtest 'format_size' => sub {
	is( FuguLib::Util::format_size(0),          '0B',    'zero' );
	is( FuguLib::Util::format_size(512),        '512B',  'bytes' );
	is( FuguLib::Util::format_size(1023),       '1023B', 'just under 1K' );
	is( FuguLib::Util::format_size(1024),       '1.0K',  'one kilobyte' );
	is( FuguLib::Util::format_size(1536),       '1.5K',  'one and a half' );
	is( FuguLib::Util::format_size(1024**2),    '1.0M',  'one megabyte' );
	is( FuguLib::Util::format_size(1024**3),    '1.0G',  'one gigabyte' );
	is( FuguLib::Util::format_size(1024**4),    '1.0T',  'one terabyte' );
	is( FuguLib::Util::format_size(1024**5),    '1024.0T',
		'past the largest unit it keeps counting' );
	is( FuguLib::Util::format_size(undef), '?',
