#!/usr/bin/env perl
# ex:ts=8 sw=4:
# Unit tests for FuguLib::EventLoop.
#
...
subtest 'run returns when nothing is left to wait for' => sub {
	...
	my $second = FuguLib::EventLoop->new;
	my $ran    = 0;
...
subtest 'an interrupt flag ends the loop' => sub {
	...
	my $signal = FuguLib::Signal->new;
	$signal->setup_interrupt_flag('USR2');

	my $loop = FuguLib::EventLoop->new( signal => $signal );
