...
sub fixture ($text)
{
	...
	return FuguLib::Config->new( file => $path );
}
...
subtest 'an unreadable file is an error, not empty defaults' => sub {
	my $config = FuguLib::Config->new( file => "$dir/no-such-file.conf" );

	is( $config->load, undef, 'load returns undef' );
	like( $config->error, qr/Cannot open/, 'and says why' );

	ok( !eval { FuguLib::Config->new; 1 }, 'new needs a file' );
};
...
subtest 'find_project_root walks up to the marker' => sub {
	...
	my $found = FuguLib::Config->find_project_root('.marker');
	my $none  = FuguLib::Config->find_project_root('.no-such-marker');
	...
	ok( !eval { FuguLib::Config->find_project_root; 1 },
