#!/usr/bin/env perl
# ex:ts=8 sw=4:
# Unit tests for the three packages in FuguLib::Proxy. The generic
...
sub cache (%args)
{
	return FuguLib::Proxy::Cache->new(
...
subtest 'cacheability is the caller decision' => sub {
	...
	my $strict = FuguLib::Proxy::Cache->new( dir => tempdir( CLEANUP => 1 ) );
...
subtest 'store, lookup and list' => sub {
	...
	is( FuguLib::File->read($path), 'content', 'with its content' );
...
subtest 'store_from_file' => sub {
	...
	FuguLib::File->write( $source, 'from a file' );
	...
	is( FuguLib::File->read($path), 'from a file', 'with the content' );
...
subtest 'the metadata table validates against the file' => sub {
	...
	my $meta = FuguLib::Proxy::Meta->new;
	...
	FuguLib::File->write( $path, 'different content of another length' );
...
subtest 'the supervisor before anything runs' => sub {
	...
	# over the same ports
	my $narrow = FuguLib::Proxy->new(
		cache   => cache(),
		pidfile => FuguLib::Pidfile->new( path => "$dir/other.pid" ),
		store   => FuguLib::Store->new( path => "$dir/other.json" )->load,
		ports   => [ 19000, 19010 ],
	);
...
subtest 'the supervisor needs its three parts' => sub {
	...
	ok( !eval { FuguLib::Proxy->new; 1 }, 'new needs arguments' );
	ok(
		!eval {
			FuguLib::Proxy->new( cache => cache() );
			1;
		},
		'a cache alone is not enough'
	);
	ok( !eval { FuguLib::Proxy::Cache->new; 1 }, 'a cache needs a dir' );
};
