#!/usr/bin/env perl
# ex:ts=8 sw=4:
# FuguLib must load with core Perl only. A module that wraps a CPAN
...
opendir my $dh, "$lib/FuguLib" or plan skip_all => "no $lib/FuguLib: $!";
my @modules = sort grep { s/\.pm$// } readdir $dh;
closedir $dh;

plan skip_all => 'no FuguLib modules found' unless @modules;
...
for my $module (@modules) {
	my $output = run("require FuguLib::$module; print qq{ok\n}");
	like( $output, qr/^ok$/m, "FuguLib::$module loads with core Perl" )
