#!/usr/bin/env perl
# ex:ts=8 sw=4:
# Unit tests for FuguLib::Control and FuguLib::Control::Client over a
...
subtest 'the registry is readable' => sub {
	my $control = FuguLib::Control->new( path => "$dir/unused.sock" );
	...
	ok( !eval { FuguLib::Control->new; 1 }, 'a server needs a path' );
	ok( !eval { FuguLib::Control::Client->new; 1 },
...
subtest 'a reply larger than one frame spans frames' => sub {
	...
	my $big = 'x' x ( 4 * FuguLib::Imsg::MAX_PAYLOAD() );
...
subtest 'a malformed request is refused' => sub {
	...
	my $seq = 100;
	my $reply = sub ( $data, $type = FuguLib::Control::TYPE_REQUEST() ) {
		my $id = $seq++;
	...
	is( $good->{type}, FuguLib::Control::TYPE_REPLY(),
...
subtest 'an absent socket is not a refusal' => sub {
	my $client =
	    FuguLib::Control::Client->new( path => "$dir/never-made.sock" );
...
subtest 'a listen refuses to take a live socket' => sub {
	...
	my $second = FuguLib::Control->new( path => $path );
