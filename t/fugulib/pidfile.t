...
{
	my $pidfile = FuguLib::Pidfile->new( path => next_path() );
	isa_ok( $pidfile, 'FuguLib::Pidfile' );
	ok( !defined $pidfile->error, 'no error on a fresh object' );
}

# Test 2: A missing path is a programming error
{
	ok( !eval { FuguLib::Pidfile->new; 1 }, 'new dies without a path' );
	ok( !eval { FuguLib::Pidfile->new( path => '' ); 1 },
...
# Test 7: An absent or malformed file yields no PID
{
	...
	my $bad = FuguLib::Pidfile->new( path => $path );
...
{
	my $pidfile =
	    FuguLib::Pidfile->new( path => "$dir/absent-dir/openhapd.pid" );
...
{
	my $path = next_path();
	my $held = FuguLib::Pidfile->new( path => $path );
	...
	if ( $pid == 0 ) {
		my $second = FuguLib::Pidfile->new( path => $path );
	...
	my $again = FuguLib::Pidfile->new( path => $path );
...
{
	my $path = next_path();
	is( FuguLib::Pidfile->new( path => $path )->path,
