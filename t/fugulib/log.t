...
# spellings are level names, not methods.
{
	...
	is( FuguLib::Log->new( mode => 'quiet', level => 'warn' )->level,
		'warning', 'the warn spelling parses as warning' );
	is( FuguLib::Log->new( mode => 'quiet', level => 'err' )->level,
		'error', 'the err spelling parses as error' );
}

# Test 9: level and mode accessors
{
	my $log = FuguLib::Log->new( mode => 'quiet', level => 'notice' );
	is( $log->level, 'notice',                 'level returns the name' );
	is( $log->mode, FuguLib::Log::MODE_QUIET(), 'mode returns the mode' );
...
# Test 10: reopen keeps the object usable and returns it
{
	...
	my $stderr_log = FuguLib::Log->new( mode => 'stderr', level => 'info' );
	$stderr_log->reopen;
...
{
	my $first = FuguLib::Log->default;
	ok( defined $first, 'default creates a logger on the first call' );
	is( FuguLib::Log->default, $first, 'default is stable' );

	my $quiet = FuguLib::Log->new( mode => 'quiet' );
	is( FuguLib::Log->set_default($quiet),
		$quiet, 'set_default returns the new default' );
	is( FuguLib::Log->default, $quiet, 'default returns what was set' );

	# A library that logs through the default must never die
	ok( eval { FuguLib::Log->default->info('through the default'); 1 },
