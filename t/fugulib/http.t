...
subtest 'parse a request' => sub {
	my $request = FuguLib::HTTP::parse_request(
	...
	is( FuguLib::HTTP::parse_request("not a request line\r\n\r\n"),
		undef, 'a line that is not a request line' );
	is( FuguLib::HTTP::parse_request(''), undef, 'an empty message' );
};

subtest 'parse a response' => sub {
	my $response = FuguLib::HTTP::parse_response(
...
subtest 'build a request' => sub {
	...
	my $bare = FuguLib::HTTP::build_request();
	...
	my $own = FuguLib::HTTP::build_request(
		body    => 'xxxx',
...
subtest 'build a response' => sub {
	my $response = FuguLib::HTTP::build_response(
		status  => 200,
	...
	my $empty = FuguLib::HTTP::build_response( status => 204 );
	...
	my $custom = FuguLib::HTTP::build_response(
		status      => 470,
	...
	like( FuguLib::HTTP::build_response( status => 470 ),
	...
	is( FuguLib::HTTP::build_response(%args),
		FuguLib::HTTP::build_response(%args),
...
subtest 'status_text' => sub {
	is( FuguLib::HTTP::status_text(200), 'OK', 'the text of 200' );
	is( FuguLib::HTTP::status_text(404), 'Not Found', 'the text of 404' );
	is( FuguLib::HTTP::status_text(413),
		'Content Too Large', 'the text of 413' );
	is( FuguLib::HTTP::status_text(999), 'Unknown', 'an unknown code' );
};
...
subtest 'framing: a message split across reads' => sub {
	...
	for my $byte ( split //, $whole ) {
		$buffer .= $byte;
		push @complete, FuguLib::HTTP::message_complete($buffer);
	}
	...
	my $middle =
	    ...
	    . "Content-Type: text/plain\r\n"
	    . "\r\n" . 'hello';
	is( FuguLib::HTTP::message_complete($middle),
	...
	is( FuguLib::HTTP::message_complete($headers_only),
		0, 'headers with a declared body are not a whole message' );

	my $parsed = FuguLib::HTTP::parse_request($whole);
...
subtest 'framing: a message over the limit' => sub {
	...
	ok( FuguLib::HTTP::message_complete( $request, max_size => 1000 ),
		'inside the limit it completes' );

	is( FuguLib::HTTP::message_complete( $request, max_size => 100 ),
	...
	is( FuguLib::HTTP::message_complete( $head, max_size => 100 ),
	...
	is( FuguLib::HTTP::message_complete( 'x' x 200, max_size => 100 ),
		undef, 'an endless header block is refused' );
	is( FuguLib::HTTP::message_complete( 'x' x 50, max_size => 100 ),
