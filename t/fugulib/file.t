...
subtest 'read and write round-trip' => sub {
	...
	ok( FuguLib::File->write( $path, "hello\n" ), 'write reports success' );
	is( FuguLib::File->read($path), "hello\n", 'read returns the bytes' );

	ok( FuguLib::File->write( $path, '' ), 'an empty write succeeds' );
	is( FuguLib::File->read($path), '', 'and reads back empty' );

	is( FuguLib::File->read("$dir/absent.txt"),
		undef, 'an absent file reads as undef' );
	is( FuguLib::File->write( "$dir/no-such-dir/x", 'x' ),
...
subtest 'binary content survives' => sub {
	...
	FuguLib::File->write( $path, $data );
	is( FuguLib::File->read($path), $data, 'every byte round-trips' );
...
subtest 'write_atomic publishes by rename' => sub {
	...
	FuguLib::File->write( $path, "old\n" );
	ok( FuguLib::File->write_atomic( $path, "new\n" ), 'atomic write' );
	is( FuguLib::File->read($path), "new\n", 'the content replaced' );
	...
	is( FuguLib::File->write_atomic( "$dir/no-such-dir/x", 'x' ),
...
subtest 'JSON files' => sub {
	...
	ok( FuguLib::File->write_json( $path, $data ), 'write_json' );
	is_deeply( FuguLib::File->read_json($path), $data, 'read_json' );

	# Canonical encoding: the same data always gives the same bytes
	my $first = FuguLib::File->read($path);
	FuguLib::File->write_json( $path, $data );
	is( FuguLib::File->read($path), $first, 'the encoding is stable' );

	is( FuguLib::File->read_json("$dir/absent.json"),
		undef, 'an absent JSON file reads as undef' );

	FuguLib::File->write( "$dir/broken.json", '{not json' );
	is( FuguLib::File->read_json("$dir/broken.json"),
		undef, 'a corrupt JSON file reads as undef' );

	FuguLib::File->write( "$dir/empty.json", '' );
	is( FuguLib::File->read_json("$dir/empty.json"),
...
subtest 'a write never exposes a secret' => sub {
	...
	FuguLib::File->write_json( $json, { password => 'hunter2' },
		mode => 0600 );
	is( mode_of($json), 0600, 'write_json publishes at 0600' );
	is( FuguLib::File->read_json($json)->{password},
...
subtest 'ensure_dir' => sub {
	...
	ok( FuguLib::File->ensure_dir($path), 'creates a nested directory' );
	ok( -d $path,                         'the directory exists' );
	ok( FuguLib::File->ensure_dir($path), 'a second call succeeds' );

	# A path that is a file is not a directory to write through
	my $file = "$dir/not-a-dir";
	FuguLib::File->write( $file, 'x' );
	is( FuguLib::File->ensure_dir($file), undef, 'refuses a plain file' );
	...
	SKIP: {
		...
		is( FuguLib::File->ensure_dir($link),
...
subtest 'expand_tilde' => sub {
	...
	is( FuguLib::File->expand_tilde('~/.cache/x'),
		'/home/tester/.cache/x', 'a leading ~ expands' );
	is( FuguLib::File->expand_tilde('~'), '/home/tester', 'a bare ~ expands' );
	is( FuguLib::File->expand_tilde('/absolute/path'),
		'/absolute/path', 'an absolute path is unchanged' );
	is( FuguLib::File->expand_tilde('relative/~/path'),
		'relative/~/path', 'a ~ in the middle is unchanged' );
	is( FuguLib::File->expand_tilde('~other/path'),
		'~other/path', 'another user is left alone' );
	is( FuguLib::File->expand_tilde(undef), undef, 'undef stays undef' );
};

subtest 'share_path finds a shipped file' => sub {
	# The module tree itself is the reference point, so a file that
	# the repository ships resolves from any working directory.
	my $found = FuguLib::File->share_path('Makefile');
	ok( defined $found, 'a shipped file resolves' );
	ok( -f $found,      'and the path exists' );

	is( FuguLib::File->share_path('no/such/file/anywhere'),
		undef, 'a missing file resolves to undef' );

	my $root = tempdir( CLEANUP => 1 );
	mkdir "$root/share";
	FuguLib::File->write( "$root/share/thing", 'x' );
	is( FuguLib::File->share_path( 'share/thing', root => $root ),
...
subtest 'atomic_dir publishes or leaves nothing' => sub {
	...
	is(
		FuguLib::File->atomic_dir( $died, sub ($) { die "nope\n" } ),
		undef, 'a die discards the build' );
	ok( !-e $died, 'and nothing is published' );

	# No leftovers in the parent
	is( FuguLib::File->sweep_temp("$dir/tree"),
		0, 'no build directories are left behind' );

	# A target that exists is not overwritten
	is( FuguLib::File->atomic_dir( $target, sub ($) { 1 } ),
		undef, 'an existing target is refused' );
	is( FuguLib::File->read("$target/base"),
		'content', 'and its content is intact' );
};

subtest 'sweep_temp removes an orphan build directory' => sub {
	my $parent = "$dir/sweep";
	FuguLib::File->ensure_dir($parent);

	mkdir "$parent/.tmp.99999.1" or die "mkdir: $!";
	mkdir "$parent/keep"         or die "mkdir: $!";

	is( FuguLib::File->sweep_temp($parent), 1, 'one orphan removed' );
	ok( !-e "$parent/.tmp.99999.1", 'the orphan is gone' );
	ok( -d "$parent/keep",          'a real entry stays' );

	is( FuguLib::File->sweep_temp("$dir/no-such-parent"),
		0, 'an absent parent sweeps nothing' );
};

subtest 'valid_name refuses what escapes a directory' => sub {
	ok( FuguLib::File->valid_name('default'),   'a plain name' );
	ok( FuguLib::File->valid_name('a-b_c.1'),   'punctuation that is safe' );
	ok( FuguLib::File->valid_name('x' x 255),   'the longest allowed name' );

	ok( !FuguLib::File->valid_name(undef),      'undef' );
	ok( !FuguLib::File->valid_name(''),         'the empty string' );
	ok( !FuguLib::File->valid_name('x' x 256),  'a name that is too long' );
	ok( !FuguLib::File->valid_name('a/b'),      'a path separator' );
	ok( !FuguLib::File->valid_name('../etc'),   'a traversal' );
	ok( !FuguLib::File->valid_name('.'),        'the current directory' );
	ok( !FuguLib::File->valid_name('..'),       'the parent directory' );
	ok( !FuguLib::File->valid_name("a\x00b"),   'an embedded NUL' );
};
