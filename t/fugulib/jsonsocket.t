#!/usr/bin/env perl
# ex:ts=8 sw=4:
# Unit tests for FuguLib::JSONSocket against a fake peer on a local
# socket.
...
subtest 'request and reply' => sub {
	...
	my $client = FuguLib::JSONSocket->new( path => $path );
...
subtest 'a greeting is read at connect when asked' => sub {
	...
	my $client =
	    FuguLib::JSONSocket->new( path => $path, greeting => 1 );
...
subtest 'a read has a wall-clock deadline' => sub {
	...
	my $client = FuguLib::JSONSocket->new( path => $path, timeout => 1 );
	$client->connect;
...
subtest 'a missing socket fails cleanly' => sub {
	my $client =
	    FuguLib::JSONSocket->new( path => "$dir/never-created.sock" );
...
subtest 'a missing path is a programming error' => sub {
	ok( !eval { FuguLib::JSONSocket->new; 1 }, 'new needs a path' );
	ok( !eval { FuguLib::JSONSocket->new( path => '' ); 1 },
		'and a real one' );
};
