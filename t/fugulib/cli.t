		...
		commands => {
			status => {
				...
				run     => sub ( $cli, @argv ) {
					...
					return FuguLib::CLI::EXIT_SUCCESS();
				},
			},
			list => {
...
subtest 'exit codes are shared constants' => sub {
	is( FuguLib::CLI::EXIT_SUCCESS(),      0, 'EXIT_SUCCESS' );
	is( FuguLib::CLI::EXIT_ERROR(),        1, 'EXIT_ERROR' );
	is( FuguLib::CLI::EXIT_INVALID_ARGS(), 2, 'EXIT_INVALID_ARGS' );
	is( FuguLib::CLI::EXIT_CONFIG_ERROR(), 3, 'EXIT_CONFIG_ERROR' );
	is( FuguLib::CLI::EXIT_TIMEOUT(),      7, 'EXIT_TIMEOUT' );
};
...
subtest 'the table must be usable' => sub {
	ok( !eval { FuguLib::CLI->new; 1 }, 'new needs a command table' );
	ok(
		!eval {
			FuguLib::CLI->new( commands => { x => {} } );
			1;
		},
